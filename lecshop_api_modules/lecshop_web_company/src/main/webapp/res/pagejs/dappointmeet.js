...
function toConfirmDoc(id, lawyerId) {
    $("#appointmentMeettingId").val(id);
    $("#appointmentlawyerId").val(lawyerId);
...
function confirmDoc() {
    LSFetch({
        ...
        data: {
            meetId: $("#appointmentMeettingId").val(),
            lawyerId: $("#appointmentlawyerId").val(),
            meetType: "1"
        },
        success: function (res) {
            if (res === 1) {
...
function changeTables(j) {
    ...
    if (j === 2) {
        window.location.href = "c_todisputesservice.htm";
    }
    if (j === 1) {
        window.location.href = location.href;
    }
