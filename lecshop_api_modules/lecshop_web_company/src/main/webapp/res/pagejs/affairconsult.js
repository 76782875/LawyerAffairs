                ...
                for (var i = 0; i < res.data.length; i++) {
                    ...
                    if (pageNum === 0 && (i === 0 || i === 1 || i === 2)) {
                        html += '<div class="panel lawyer_mian"> ' +
                            ...
                            '                                <button type="button" class="btn btn-info" onclick="appointMeet(' + lawyerId + ')"><i class="icon-calendar"></i> 预约会面 ' +
                            '                                </button> ' +
                            '                            </div> ' +
                            '                        </div> ' +
                        ...
                        htmls += '<div class="col-md-4 col-sm-6">' +
                            ...
                            '                                        <button type="button" class="btn btn-info" onclick="appointMeet(' + lawyerId + ')"><iclass="icon-calendar"></i> 预约' +
                            '                                        </button>' +
                            '                                    </div>' +
                            '                                </div>' +
                            '                            </div>' +
...
function changeType(obj, name) {
    $(obj).find("option:selected").val() !== "" ? loadTwoType($(obj).find("option:selected").attr("data-id"), name) : loadTwoType("-2", name);
}


function toPastCases(code, name, institution) {
    var code = code === null ? "" : code;
    window.location.href = basePath + "c_topastcases.htm" + "?code=" + code + "&name=" + encodeURIComponent(encodeURIComponent(name)) + "&institution=" + encodeURIComponent(encodeURIComponent(institution)) + "&type=" + 1;
}
