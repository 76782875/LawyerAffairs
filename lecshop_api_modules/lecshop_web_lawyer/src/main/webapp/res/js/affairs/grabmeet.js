...
function fetchData() {
    $.ajax({
        url: basePath + '/querygrabmeets.htm',
        type: 'post',
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.data.length; i++) {
                ...
                htmls += '<div class="card ks-card-header-pic">' +
                    ...
                    '<p>状态：' + letterStatus[res.data[i].status] + '</p> </div> </div><div class="card-footer" style="justify-content:flex-end">' + applyhtml +
                    '</div> </div>';
            }
...
function torefuse(id) {
    myApp.confirm('确定?', "提示",
        function () {
            $.ajax({
                url: basePath + '/refusecompanymeet.htm?id=' + id,
                type: 'post',
                data: {'pageNum': pageNum},
                success: function () {
...
function toconfirm(id) {
    myApp.confirm('确定?', "提示",
        function () {
            $.ajax({
                url: basePath + '/confirmcompanymeet.htm?id=' + id,
                type: 'post',
                data: {'pageNum': pageNum},
                success: function () {
                    window.location.reload();
