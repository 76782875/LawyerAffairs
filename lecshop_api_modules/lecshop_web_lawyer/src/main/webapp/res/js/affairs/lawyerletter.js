...
function fetchData() {
    $.ajax({
        url: basePath + '/querylawyerletters.htm',
        type: 'post',
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.data.length; i++) {
                ...
                if (res.data[i].status != '0') {
                    uploadhtml = '<a href="#" onclick="toupload(' + res.data[i].id + ')" class="button active qd_btn">上传律师函</a>';
                }
                ...
                htmls += '<div class="card ks-card-header-pic">' +
                    ...
                    '<p>收件人电话：' + res.data[i].mobile + '</p>' +
                    '<p>收件人地址：' + res.data[i].address + '</p>' +
...
function toconfirm(id) {
    myApp.confirm('确定?', "提示",
        function () {
            $.ajax({
                url: basePath + '/confirmletter.htm?letterId=' + id,
                type: 'post',
                data: {'pageNum': pageNum},
                success: function () {
...
function toupload(id) {
    window.location.href = basePath + "/touploadlawyerletter.htm?letterId=" + id
}

// 跳转到详情页
function todetail(id) {
    window.location.href = basePath + "/tolawyerletterdetails.htm?letterId=" + id
}