...
function fetchData() {
    $.ajax({
        url: basePath + '/querymodifydocs.htm',
        type: 'post',
...
function toconfirm(id) {
    myApp.confirm('确定?', "提示",
        function () {
            $.ajax({
                url: basePath + '/updatemodifydocstatus.htm?docId=' + id,
                type: 'post',
                success: function () {
...
function toupload(id) {
    window.location.href = basePath + "/touploadmodifydoc.htm?docId=" + id
}

// 跳转到详情页
function todetail(id) {
    window.location.href = basePath + "/tomodifydocdetails.htm?docId=" + id
}