...
<body>
<div class="views">
  <input type="hidden" value="${letterId}" id="letterId">
...
<script>
  ...
  function uploadLetter() {
    ...
    if ($("#desc").val() == "" && $("#xFileText").html() == "") {
      window.location.href = "<%=basePath %>/tolawyerletterdetails.htm?letterId=" + $("#letterId").val();
      return ;
    }

    $.ajax({
      url: '<%=basePath %>/addlawyerletterdetail.htm',
        type: 'post',
        data: {
          'letterId': $("#letterId").val(),
          'desc': $("#desc").val(),
          'fileUrl': $("#xFileText").html()
        },
